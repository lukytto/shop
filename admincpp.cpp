...
void Admin::AddUser() { // Add new user
	...
	for (size_t i = 0; i < lib.UserArray.size(); i++)
		if (lib.UserArray[i].GetUsername() == name) { // If find that username
			if (lib.UserArray[i].GetStatus() != 0) { // Check status, 0 doesnt exist
...
void Admin::ListUser() { // List all users' information
	...
	temp = "Customer";
	Stat.push_back(temp);
	cout << "This is the list of all users:" << endl;
	cout << "-------------------------------------------------------------------------" << endl;
...
void Admin::Interface(string username) { // Admin interface
	...
	while (1) {
		...
		while (1) { // The operation list
			...
			cout << "6 ----- Change item information" << endl;
			cout << "7 ----- List items (ALL)" << endl;
			cout << "8 ----- List items (By Category)" << endl;
			cout << "9 ----- List items (By Name)" << endl;
			cout << "* ----- Show more information about item by ID number" << endl;
